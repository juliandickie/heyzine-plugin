...
export function rowToSpec(row) {
  const design = {};
  for (const col of DESIGN_COLUMNS) { const b = coerceBool(row[col]); if (b !== undefined) design[col] = b; }
  ...
  return {
    ...
    replace: coerceBool(row.replace) === true, tags: row.tags || undefined, note: row.note || undefined, description: row.description || undefined,
    embedded: row.embedded || undefined, design,
  };
}

export const RESULT_COLUMNS = ['row', 'name', 'status', 'id', 'short', 'url', 'base', 'pages', 'error'];
...
export async function runBatch(ctx, rows, { concurrency = 2, cache = null, publish = publishOne, onRow = () => {} } = {}) {
  ...
  async function worker() {
    for (;;) {
      ...
      if (existing) {
        result = { row: index + 1, name: spec.name, status: 'skipped-existing', id: existing.id, short: shortId(existing.id), url: publicUrl(existing, ctx.settings?.publicHost ?? ''), base: existing.links?.base ?? '', pages: existing.pages ?? '', error: '' };
      } else {
        try {
          const r = await publish(ctx, spec);
          result = { row: index + 1, name: spec.name, status: spec.replace ? 'replaced' : 'converted', id: r.id, short: r.short, url: r.url, base: r.base, pages: r.pages ?? '', error: '' };
        } catch (error) {
          result = { row: index + 1, name: spec.name, status: 'failed', id: '', short: '', url: '', base: '', pages: '', error: `${error.code ?? 'error'}: ${error.message}` };
        }
      }
  ...
  await Promise.all(Array.from({ length: Math.max(1, Math.min(concurrency, rows.length || 1)) }, worker));
