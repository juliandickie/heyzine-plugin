...
import { publicUrl, shortId, parseId, driveDirectUrl, linkConversionUrl, isFullFlipbookId, isFullBookshelfId } from './links.mjs';
import { parseNote, tagFacets } from './register.mjs';
import { refreshInventory, loadInventory, inventoryPath, resolveFullId, reconcile, parseNamesFile } from './inventory.mjs';
...
const BOOL_FLAGS = ['json', 'wait', 'replace', 'refresh', 'yes', 'csv', 'help', 'skip-verify'];
...
export function parseBool(value) {
  ...
  throw new HeyzineError('validation', `Expected true or false, got "${value}"`);
}
...
async function readInventory(ctx) {
  try {
    return await loadInventory({ dir: ctx.dataDir, readFile: ctx.readFile });
  } catch (error) {
    throw new HeyzineError('validation', `Inventory cache is unreadable (${error.message}) - run: heyzine inventory --refresh`);
  }
}
...
async function preflightSource(ctx, source) {
  ...
  if (result.status >= 400) throw new HeyzineError('validation', `Source URL answered HTTP ${result.status}: ${url}`);
...
async function resolveId(ctx, input, expectKind) {
  ...
  if (!s) throw new HeyzineError('validation', 'An id, short id or URL is required');
  if (isFullFlipbookId(s) || isFullBookshelfId(s)) return s.toLowerCase();
  ...
  if (!hit) {
    ...
    if (kind !== 'bookshelf') live.items = await ctx.client.listFlipbooks();
  ...
  if (expectKind && hit.kind !== expectKind) throw new HeyzineError('validation', `"${s}" is a ${hit.kind}, expected a ${expectKind}`);
  return hit.full;
}

function requireArg(positionals, index, what) {
  const v = positionals[index];
  if (v === undefined || v === '') throw new HeyzineError('validation', `Missing argument: ${what}`);
  return v;
}
...
export const COMMANDS = {
  ...
  list: { usage: 'list [--limit N] [--offset N]', needsKey: true, run: async (ctx, p, f) => (await ctx.client.listFlipbooks({ limit: f.limit, offset: f.offset })).map((i) => withPublic(i, ctx.settings.publicHost)) },
  ...
  publish: { usage: 'publish <source> --name <name> [--purpose P] [--course C] [--idd-to slug] [--template id] [--replace] [--note text] [--description text] [--embedded list] [--skip-verify] [design flags]', needsKey: true, run: async (ctx, p, f) => {
    if (!f.name) throw new HeyzineError('validation', '--name is required (the resource name that becomes the title)');
  ...
  design: { usage: 'design <id> [design flags]', needsKey: true, run: async (ctx, p, f) => {
    const id = await resolveId(ctx, requireArg(p, 0, 'flipbook id'), 'flipbook');
    const fields = designFields(f);
    if (!Object.keys(fields).length) throw new HeyzineError('validation', 'No design fields given');
  ...
  'shelf-add': { usage: 'shelf-add <shelf> <flipbook> [--position N]', needsKey: true, run: async (ctx, p, f) => ctx.client.addToBookshelf(await resolveId(ctx, requireArg(p, 0, 'bookshelf id'), 'bookshelf'), await resolveId(ctx, requireArg(p, 1, 'flipbook id'), 'flipbook'), f.position === undefined ? undefined : Number(f.position)) },
  ...
  'access-setup': { usage: 'access-setup <id> --mode disabled|everyone|users [--type flipbook|bookshelf] [--password P] [--text-user T] [--text-password T]', needsKey: true, run: async (ctx, p, f) => {
    if (!f.mode) throw new HeyzineError('validation', '--mode is required (disabled, everyone, users)');
    const id = await resolveId(ctx, requireArg(p, 0, 'flipbook or bookshelf id'), f.type === 'bookshelf' ? 'bookshelf' : undefined);
    return ctx.client.accessSetup(compact({ id, type: f.type, mode: f.mode, password: f.password, text_user: f['text-user'], text_password: f['text-password'] }));
  } },
  'access-add': { usage: 'access-add <id> --access-type T [--user U] [--password P] [--type flipbook|bookshelf]', needsKey: true, run: async (ctx, p, f) => {
    if (!f['access-type']) throw new HeyzineError('validation', '--access-type is required');
    const id = await resolveId(ctx, requireArg(p, 0, 'flipbook or bookshelf id'), f.type === 'bookshelf' ? 'bookshelf' : undefined);
    return ctx.client.accessAdd(compact({ id, access_type: f['access-type'], user: f.user, password: f.password, type: f.type }));
  } },
  'access-remove': { usage: 'access-remove <id> [--user U] [--password P] [--type flipbook|bookshelf]', needsKey: true, run: async (ctx, p, f) => {
    const id = await resolveId(ctx, requireArg(p, 0, 'flipbook or bookshelf id'), f.type === 'bookshelf' ? 'bookshelf' : undefined);
    return ctx.client.accessRemove(compact({ id, user: f.user, password: f.password, type: f.type }));
  } },
  search: { usage: 'search <query>', needsKey: true, run: async (ctx, p) => ctx.client.searchText(requireArg(p, 0, 'query')) },
  'page-text': { usage: 'page-text <id> <page>', needsKey: true, run: async (ctx, p) => ctx.client.pageText(await resolveId(ctx, requireArg(p, 0, 'flipbook id'), 'flipbook'), Number(requireArg(p, 1, 'page number'))) },
  oembed: { usage: 'oembed <flipbook url> [--maxwidth N] [--maxheight N]', needsKey: true, run: async (ctx, p, f) => ctx.client.oembed(requireArg(p, 0, 'flipbook URL'), { maxwidth: f.maxwidth, maxheight: f.maxheight }) },
  ...
  mcp: { usage: "mcp <tool name> [--args '{json}']", needsKey: true, run: async (ctx, p, f) => ctx.client.mcpCall(requireArg(p, 0, 'tool name'), f.args ? JSON.parse(f.args) : {}) },
  ...
  reconcile: { usage: 'reconcile <names file> [--csv]', needsKey: true, run: async (ctx, p) => {
    const cache = await readInventory(ctx);
    if (!cache) throw new HeyzineError('validation', 'No inventory cache yet - run: heyzine inventory --refresh');
    const names = parseNamesFile(await ctx.readFile(requireArg(p, 0, 'names file'), 'utf8'));
    return reconcile(names, cache, { publicHost: ctx.settings.publicHost });
  } },
  batch: { usage: 'batch <csv> [--yes] [--concurrency N] [--skip-verify]', needsKey: true, run: async (ctx, p, f) => {
    const file = requireArg(p, 0, 'batch csv');
    const rows = parseCsv(await ctx.readFile(file, 'utf8'));
    if (!rows.length) throw new HeyzineError('validation', `${file} has no rows`);
    ...
    const { results, summary } = await runBatch({ ...ctx, skipVerify }, rows, {
      concurrency: f.concurrency ? Number(f.concurrency) : 2,
      cache,
...
export async function main(argv, deps = {}) {
  const {
    ...
    makeClient = (key) => new HeyzineClient({ key, fetch }), today = () => new Date().toISOString().slice(0, 10), sleep,
  } = deps;
  ...
  try {
    ...
    const ctx = { settings, dataDir: dataDir({ env, homedir }), stdout, stderr, fetch, readFile, writeFile, mkdir, today, sleep, pollOptions: { sleep } };
    if (command.needsKey) {
    ...
    if (error instanceof ConfigError || code === 'config') return 2;
