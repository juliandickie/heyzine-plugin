...
export class HeyzineClient {
  constructor({ key, fetch = globalThis.fetch, sleep = defaultSleep, backoffMs = [1000, 3000, 9000], baseUrl = 'https://heyzine.com/api1', mcpUrl = 'https://heyzine.com/mcp' } = {}) {
  ...
  async #send(url, init, endpoint) {
    ...
    for (;;) {
      ...
      try {
        ...
        if (attempt < this.backoffMs.length) { await this.sleep(this.backoffMs[attempt++]); continue; }
        throw new HeyzineError('transient', `Network error calling ${endpoint}: ${error.message}`, { retryable: true, endpoint });
      }
      if (TRANSIENT_STATUS.has(res.status) && attempt < this.backoffMs.length) { await this.sleep(this.backoffMs[attempt++]); continue; }
      let body;
  ...
  async request(method, endpoint, { query, body } = {}) {
    ...
    const res = await this.#send(url.toString(), init, endpoint);
  ...
  convertSync(params) { return this.request('POST', 'rest', { body: params }); }
  ...
  async mcpCall(name, args = {}) {
    const init = {
      method: 'POST',
      headers: { Authorization: `Bearer ${this.key}`, 'Content-Type': 'application/json', Accept: 'application/json', 'MCP-Protocol-Version': '2025-06-18' },
