...
export class HeyzineClient {
  ...
  async #send(url, init, endpoint, backoffMs = this.backoffMs) {
    let attempt = 0;
    for (;;) {
      let res;
      let text;
  ...
  async request(method, endpoint, { query, body, backoffMs } = {}) {
    ...
    const res = await this.#send(url.toString(), init, endpoint, backoffMs ?? this.backoffMs);
