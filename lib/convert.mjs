import { assertNote } from './register.mjs';
...
export function convertParams({ pdf, clientId, ...rest } = {}) {
  ...
  if (params.private_note !== undefined) assertNote(params.private_note);
  return params;
}
...
export async function convertAndWait(client, params, { intervalMs = 5000, timeoutMs = 900000, sleep = defaultSleep, now = Date.now, onPoll = () => {} } = {}) {
  ...
  for (;;) {
    ...
    if (state === 'failed') {
      throw new HeyzineError('conversion_failed', `Heyzine reported the conversion failed for ${safeUrl(params.pdf)}${last?.msg ? ` - ${last.msg}` : ''}`, { body: last });
    }
    if (now() - started >= timeoutMs) {
      throw new HeyzineError('timeout', `Conversion still ${state || 'pending'} after ${Math.round(timeoutMs / 1000)} s for ${safeUrl(params.pdf)} (id ${last?.id ?? 'unknown'}) - poll again later with the same URL, the conversion continues on Heyzine's side`, { body: last, retryable: true });
    }
