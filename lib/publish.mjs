...
import { buildRegister, formatNote, parseNote, parseTags, FACETS } from './register.mjs';
...
export async function publishOne(ctx, spec) {
  ...
  const privateNote = spec.note ? formatNote({ fields: parseNote(register.private_note).fields, other: parseNote(spec.note).other }) : register.private_note;
