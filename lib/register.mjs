...
// Heyzine stores at most 200 characters of private note (probed 2026-09-15 - 200 is accepted,
// 201 answers HTTP 500 on flipbook-design, and the async convert silently drops the title,
// tags, note and template when the note is over the cap).
export const NOTE_MAX = 200;
// Lines dropped first when a generated note would not fit, least valuable first.
export const NOTE_DROP_ORDER = ['published_by', 'source_url', 'source_name'];
...
export function assertNote(text, where = 'private note') {
  const len = String(text ?? '').length;
  if (len > NOTE_MAX) throw new HeyzineError('validation', `${where} is ${len} characters, Heyzine keeps at most ${NOTE_MAX} - shorten it (the register trims source_url, source_name and published_by on its own)`);
  return text;
}

// Build the note and drop optional lines (NOTE_DROP_ORDER) until it fits NOTE_MAX. Returns the
// note and the names of the lines dropped so the caller can say so.
export function fitNote({ fields = {}, other = [] } = {}) {
  const kept = { ...fields };
  const dropped = [];
  let note = formatNote({ fields: kept, other });
  for (const key of NOTE_DROP_ORDER) {
    if (note.length <= NOTE_MAX) break;
    if (kept[key] === undefined) continue;
    delete kept[key];
    dropped.push(key);
    note = formatNote({ fields: kept, other });
  }
  return { note, dropped };
}
...
export function buildRegister({ purpose, course, iddTo, sourceDriveId, sourceName, sourceUrl, embedded, today, extraTags = '' } = {}) {
  ...
  return { tags: mergeTags(extraTags, tags), private_note: note, note_dropped: dropped };
}
